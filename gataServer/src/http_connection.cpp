...
auto HttpConnection::Start() -> void {
  ...
  http::async_read(
      ...
      [self](boost::beast::error_code ec, size_t bytes_transferred) {
        try {
          if (ec) {
            DEBUG_LOG("http read error:%s");
          }
          boost::ignore_unused(bytes_transferred);
          self->HandleReq();
          self->CheckDeadline();
        } catch (std::exception &e) {
          DEBUG_LOG("http async_read_callback error:%s", e.what());
        }
      });
}
...
void HttpConnection::PreParseGetParam() {
  // 提取 URI  ....?key1=value1&key2=value2
  auto uri = request_.target().to_string();
  // 查找查询字符串的开始位置（即 '?' 的位置）
...
auto HttpConnection::HandleReq() -> void {
  ...
  if (request_.method() == http::verb::get) {
    ...
    bool success = LogicSystem::GetInstance()->HandlePost(
        request_.target().to_string(), shared_from_this());
    if (!success) {
      response_.result(http::status::not_found);
      response_.set(http::field::content_type, "text/plain");
      beast::ostream(response_.body()) << "url not found\r\n";
